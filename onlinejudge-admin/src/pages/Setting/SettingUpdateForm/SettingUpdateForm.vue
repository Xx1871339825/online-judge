<template>
  ...
  <el-button :loading="settingFormReactive.isLoading" @click="clickAdd" type="primary">修改</el-button>
...
import {getSetting, updateSetting} from "../../../axios/network/setting";
...
onMounted(()=>{
  imageUrl.value = ''
  if (reactiveProps.sid != -1){
    getSetting(reactiveProps.sid).then(res => {
      const {data} = res.data
      if (data.settingType == 0){
        imageUrl.value = data.content
      }
      settingFormReactive.settingId = data.settingId
      settingFormReactive.title = data.title
      settingFormReactive.sort = data.sort
      settingFormReactive.content = data.content
      const setting:any = settingType.find(item => item.id == data.settingType)
      const status:any = statusArr.find(item => item.id == data.status)
      settingFormReactive.status = status.name
      settingFormReactive.settingType = setting.name
    })
  }
})

watch(()=>reactiveProps.sid,()=>{
  imageUrl.value = ''
  if (reactiveProps.sid != -1){
    getSetting(reactiveProps.sid).then(res => {
      const {data} = res.data
...
const clickAdd = () => {
  ...
  updateSetting(settingFormReactive).then(() => {
