...
export const checkUserOrEmail = (rule: any, value: any, callback: any) => {
    if (value === null || value === ''){
        callback(new Error('用户名不能为空'))
    }else if (value.indexOf('@') === -1 && !accountReg.test(value)){
        callback(new Error('账号只能由大小写字母开头+数字组成，长度为6-18位'))
    }else if (value.indexOf('@') !== -1 && !emailReg.test(value)){
...
export const checkPassword = (rule: any, value: any, callback: any) => {
    if (value === null || value === '') {
        ...
        callback(new Error('密码只能由大小写字母和数字组成，长度为6-18位'))
    }else {
        callback()
    }
}
